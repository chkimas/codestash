...
export async function deleteSnippetAsAdmin(snippetId: string) {
...
export async function updateUserBan(userId: string, banType: BanDuration) {
  const supabase = await createAdminClient()
  ...
  switch (banType) {
    ...
    case 'permanent':
      ...
      break
    case '1h':
      updateData = { is_banned: false, banned_until: addHours(new Date(), 1).toISOString() }
      break
    case '24h':
      updateData = { is_banned: false, banned_until: addDays(new Date(), 1).toISOString() }
      break
    case '7d':
      updateData = { is_banned: false, banned_until: addDays(new Date(), 7).toISOString() }
      break
    case '30d':
      updateData = { is_banned: false, banned_until: addDays(new Date(), 30).toISOString() }
      break
  }
